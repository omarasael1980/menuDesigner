...
function checkSessionTime() {
    ...
    if (isset($_SESSION['start_time'])) {
        ...
        if (time() - $_SESSION['start_time'] > 3000) {
            ...
            header("Location: ../../index.php?tipo=info&&msg=Tu sesión ha expirado"); // Redirigir a la página de inicio de sesión
            exit();
        }
    } else {
        // Si la sesión no está iniciada, redirigir a la página de inicio de sesión
        header("Location: ../../index.php?tipo=info&&msg=Inicia sesión para continuar");
        exit();
    }
}
?>
